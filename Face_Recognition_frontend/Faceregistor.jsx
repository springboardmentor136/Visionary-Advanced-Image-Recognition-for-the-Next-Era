...
const FaceRegistration = () => {
  ...
  useEffect(() => {
    const detectFace = async () => {
      ...
      if (detections) {
        ...
        ctx.fillStyle = "red";
        ctx.font = "16px Arial";
        ctx.fillText(`Confidence: ${confidence.toFixed(2)}%`, box.x, box.y - 10);
  ...
  const handleCaptureFace = (video, box) => {
    ...
    const croppedFaceImage = tempCanvas.toDataURL("image/png"); // PNG format
...
const handleRegister = async () => {
    ...
    const faceData = {
        name,
        image: capturedImage // Only storing face image (PNG format)
    };

    localStorage.setItem("faceData", JSON.stringify(faceData));
    console.log("✅ Face registered with cropped PNG image:", faceData);
        ...
        <div className="flex flex-col items-center sm:flex-row lg:gap-x-14 sm:gap-x-8 sm:ml-32">
          ...
          <div className="flex flex-col space-y-4 w-full max-w-sm relative">
            ...
            <button
              ...
              className={`bg-green-500 text-white py-2 rounded-md mt-2 sm:w-[130px] ${isFaceDetected?"cursor-pointer":"cursor-not-allowed blur-sm"}`}
